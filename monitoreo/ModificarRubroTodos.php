...
<?
	switch ($var_rubro) 
	{
	    case 1:
			...
			$sw2 = false;
			$sw3 = false;
			$var_rubro_4 = 0.00; /*NUEVO RUBRO 104 E/R*/
			$var_rubro_5 = 0.00;
			$var_rubro_7 = 0.00; /*RUBRO 7*/
			$var_rubro_10 = 0.00;
			$var_rubro_11 = 0.00;
			$var_rubro_12 = 0.00;
			$var_rubro_14 = 0.00;
			...
			for($j=0;$j<$i;$j++)
			{
				...
				if($var_rubro_id == 4 )
				{
					$var_rubro_4 = $var_rubro_4 + $cajatexto;
				}
				if($var_rubro_id == 7 )
				{
					$var_rubro_7 = $var_rubro_7 + $cajatexto;
				}

				if($var_rubro_id == 10 )
				{
					$var_rubro_10 = $var_rubro_10 + $cajatexto;
				}
				...
				switch ($var_rubro_id)
				{
					...
					case 4:
						$query="update C_RUBRO_DATOS SET RUBRO_VALOR=$var_rubro_4 WHERE CODIGO_SKU=$codigo";
					break;					
					case 5:
						$query="update C_RUBRO_DATOS SET RUBRO_VALOR=$var_rubro_5 WHERE CODIGO_SKU=$codigo";
						break;
					case 7:
						$query="update C_RUBRO_DATOS SET RUBRO_VALOR=$var_rubro_7 WHERE CODIGO_SKU=$codigo";
						break;
					case 10:
						$query="update C_RUBRO_DATOS SET RUBRO_VALOR=$var_rubro_10 WHERE CODIGO_SKU=$codigo";
						break;										
					case 11:
			...
			{
				$sw1 = false;
				$var_graba1 = " OTROS INGRESOS OPERACIONALES, DEBE SER EN POSITIVO ;";
			}	
			...
			{
				$sw2 = false;
				$var_graba2 = " GASTOS DE PROVISIONES DE CARTERA, DEBE SER EN POSITIVO ;";
			}	
			...
			{
				$sw3 = false;
				$var_graba3 = " OTROS GASTOS OPERACIONALES Y FINANCIEROS, DEBE SER EN POSITIVO ;";
			}	
			
			/*FIN DE NUEVOS RUBROS CREADOS*/
			if($sw1 && $sw2 && $sw3)
			{
				$sw = true;
			}
	    ...
	    case 2:
			...
			for($j=0;$j<$i;$j++)
			{
				...
				switch ($var_rubro_id)
				{
					...
					case 25:
						//$var_rubro_22 = $var_rubro_308 + $var_rubro_309 + $var_rubro_310 + $var_rubro_311 + $var_rubro_312 + $var_rubro_315;
						$query="update C_RUBRO_DATOS SET RUBRO_VALOR=$var_rubro_25 WHERE CODIGO_SKU=$codigo";
						break;
			...
			{
				$sw5 = false;
				$var_graba5 = " INTERESES Y COMISIONES DE LA CARTERA POR COBRAR, DEBE SER EN POSITIVO ;";
			}	
			...
			{
				$var_graba = "SUS DATOS SE GRABARON EXISTE ERROR Y NO SERÁN PUBLICADOS:".$var_graba1.$var_graba2.$var_graba3.$var_graba4.$var_graba5;
				$var_longitud = strlen($var_graba) - 2;
				$var_graba = substr($var_graba,0,$var_longitud);
				?>
					<span class="Mal"><?php echo $var_graba;?></span>
	    ...
	    case 3:
			...
			$var_total_oficiales_mujeres_periodo_anterior = 0;
			...
			$consulta="SELECT * FROM C_RUBRO_DATOS WHERE PERIODO_ID = $var_periodo_anterior AND ORGANIZACION_ID = $var_organizacion AND RUBRO_ID = 196";
			$resultado=ConsultarTabla($consulta);
            $filas = NumeroFilas($resultado);
            if ($filas > 0)
			$var_educacion_financiera_periodo_anterior = mssql_result($resultado,0,"RUBRO_VALOR");

			$consulta="SELECT * FROM C_RUBRO_DATOS WHERE PERIODO_ID = $var_periodo_anterior AND ORGANIZACION_ID = $var_organizacion AND RUBRO_ID = 197";
			$resultado=ConsultarTabla($consulta);
            $filas = NumeroFilas($resultado);
            if ($filas > 0)
			$var_total_oficiales_mujeres_micro_periodo_anterior = mssql_result($resultado,0,"RUBRO_VALOR");
			...
			for($j=0;$j<$i;$j++)
			{
				...
				$cajatexto=$_POST[$var1];
				$var_rubro_id = $_POST[$var2];
				if($var_rubro_id == 340 || $var_rubro_id == 341 || $var_rubro_id == 342)
				{
					$var_total_tasas_comercial_prioritario = $var_total_tasas_comercial_prioritario + $cajatexto;
				}
				if($var_rubro_id == 340 || $var_rubro_id == 341 || $var_rubro_id == 342)
				{
					$var_total_tasas_comercial_prioritario = $var_total_tasas_comercial_prioritario + $cajatexto;
			...
			{	
				...
				{
					$sw74 = false;
					$var_graba74 = " EL MONTO DE COLOCACIÓN DE CARTERA DE CRÉDITO NO PUEDE SER MAYOR AL MONTO DEL ANTERIOR PERIODO ;";				
				}
			}
			else
			{
			...
			{	
				...
				{
					$sw75 = false;
					$var_graba75 = " EL VALOR DE LOS PRÉSTAMOS CASTIGADOS NO PUEDE SER MAYOR AL MONTO DEL ANTERIOR PERIODO ;";				
				}
			}
			else
			{
			...
			{	
				...
				{
					$sw76 = false;
					$var_graba76 = " EL VALOR DE LOS PRÉSTAMOS CASTIGADOS MICRO NO PUEDE SER MAYOR AL MONTO DEL ANTERIOR PERIODO ;";				
				}			}
			else
			{
				if (round($var_total_prestamos_castigados_micro,2) > round($var_total_prestamos_castigados_micro_periodo_anterior,2)) 
			...
			if (round($var_total_clientes_micro,2) < round($var_total_clientes,2)) {
				...
				{
					$sw83 = false;
					$var_graba83 = " EL NÚMERO DE PRÉSTAMOS VIGENTES MICRO NO PUEDE SER IGUAL AL NÚMERO DE PRÉSTAMOS TOTALES ;";				
				}				
			}

			if (round($var_total_oficiales_mujeres_periodo_anterior,2) == 0) 
			{
				$sw84 = true;
				$var_graba84 = "";	
			}
			...
			{
				if (round($var_total_oficiales_mujeres,2) > 0) 
				{
					$sw84 = true;
					$var_graba84 = "";	
					}
				...
				{
					$sw84 = false;
					$var_graba84 = " SE DEBE REGISTRAR EL TOTAL DE OFICIALES DE CRÉDITO MUJERES ;";				
				}
			}					

			if (round($var_total_oficiales_mujeres_micro_periodo_anterior,2) == 0) 
			{
				$sw85 = true;
				$var_graba85 = "";	
			}
			...
			{
				if (round($var_total_oficiales_mujeres_micro,2) > 0) 
				{
					$sw85 = true;
					$var_graba85 = "";	
					}
				...
				{
					$sw85 = false;
					$var_graba85 = " SE DEBE REGISTRAR EL TOTAL DE OFICIALES DE CRÉDITO MUJERES MICRO ;";				
				}
			}					
...
<table width="100%" border="0">
  <tr>
    <td><div align="center"><a class="btn ing gray" href="IngresoRubroTodos.php?var_rubro=<?php echo $var_rubro ?>"><?php echo $var_titulo ?></a></div></td>    
  </tr>
</table>
<br>
<br />
